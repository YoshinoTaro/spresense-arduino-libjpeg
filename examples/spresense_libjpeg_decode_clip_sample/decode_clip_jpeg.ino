...
bool decode_clip_jpeg(uint8_t* imgbuf, uint32_t imgsize, uint8_t* outbuf
  ,uint16_t x_offset, uint16_t y_offset, uint16_t x_size, uint16_t y_size) {
  ...
  if (x_offset + x_size >= cinfo.output_width
   || y_offset + y_size >= cinfo.output_height) {
    printf("the specified clip size is over the original jpeg image\n");
    printf("x_offset + x_size:   %d\n", x_offset + x_size);  
    printf("y_offset + y_size:   %d\n", y_offset + y_size);  
    return false;
  }   
  ...
  while (cinfo.output_scanline < cinfo.output_height) {
    ...
    if (cinfo.output_scanline <  y_offset 
     || cinfo.output_scanline >= y_offset + y_size) {
      continue; /* just read to skip the line */
    } else {
      memcpy(tmp, &buffer[0][x_offset*3], x_size*3);
      tmp += x_size*3; 
      ++n;
    }
  }
